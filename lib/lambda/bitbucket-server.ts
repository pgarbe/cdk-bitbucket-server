...
export class BitBucketServer {
  ...
  private performRequest<TReq extends BitBucketRequest, TRes extends BitBucketResponse>(method: HttpMethod, path: string, body?: TReq): Promise<TRes> {
  ...
  public createWebHook(projectKey: string, repositorySlug: string, req: BitBucketCreateWebHookRequest): Promise<BitBucketCreateWebHookResponse> {
    const repoPath = `rest/api/1.0/projects/${projectKey}/repos/${repositorySlug}/webhooks`;

    return this.performRequest<BitBucketCreateWebHookRequest, BitBucketCreateWebHookResponse>(HttpMethod.POST, repoPath, req);
  }
